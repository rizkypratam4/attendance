...
@section('content')
    ...
    {{-- Feedback --}}
    @if (session('success'))
        <div class="mb-4 p-4 rounded-xl flex items-start gap-3"
    ...
    @if (session('import_success'))
        <div class="mb-4 p-4 rounded-xl" style="background:rgba(34,197,94,.1);border:1px solid rgba(34,197,94,.3)">
            <p style="font-size:13px;color:#22c55e;font-weight:500">Berhasil import {{ session('import_success') }} baris.
            </p>
        </div>
    @endif
    ...
    <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-6">
        <div>
            <h1 style="font-size:24px;font-weight:800;color:var(--text-1);line-height:1.2">Penugasan Shift</h1>
            <p style="font-size:13px;color:var(--text-3);margin-top:5px">Tugaskan karyawan ke shift dan jadwal tertentu secara efisien.</p>
            @php
                $startOfWeek = \Carbon\Carbon::now()->startOfWeek(\Carbon\Carbon::MONDAY);
                $endOfWeek = \Carbon\Carbon::now()->endOfWeek(\Carbon\Carbon::SUNDAY);
            @endphp
            <div
                style="margin-top:8px;padding:6px 12px;border-radius:8px;background:rgba(124,58,237,.1);border:1px solid rgba(124,58,237,.2);display:inline-flex;align-items:center;gap:8px">
                ...
                <span style="font-size:12px;font-weight:600;color:#a78bfa">Minggu Ini: {{ $startOfWeek->format('d M') }} –
                    {{ $endOfWeek->format('d M Y') }}</span>
            </div>
        </div>
        <div class="flex items-center gap-2 flex-shrink-0">
            <button onclick="openM('mAssignShift')" class="flex items-center gap-2 px-4 py-2.5 rounded-xl font-semibold"
    ...
    {{-- SEARCH + FILTER --}}
    <x-search-filter searchId="assignSearch" searchPlaceholder="Cari karyawan..." :filters="[
        ['id' => 'department', 'label' => 'Departemen', 'options' => $departments->pluck('name', 'id')->toArray()],
        ['id' => 'shift_code', 'label' => 'Kode Shift', 'options' => $shiftCodes->pluck('code', 'id')->toArray()],
    ]" />
                ...
                <thead>
                    <tr style="border-bottom:1px solid var(--border)">
                        ...
                        <th class="text-left px-4 py-3 font-semibold whitespace-nowrap"
                            ...
                            Shift Code</th>
                        <th class="text-left px-4 py-3 font-semibold whitespace-nowrap"
                            style="font-size:10.5px;color:var(--text-3);letter-spacing:.08em;text-transform:uppercase">New
                            Shift</th>
                        <th class="text-left px-4 py-3 font-semibold whitespace-nowrap"
                            style="font-size:10.5px;color:var(--text-3);letter-spacing:.08em;text-transform:uppercase">
                            Periode</th>
                        <th class="text-left px-4 py-3 font-semibold whitespace-nowrap"
                            style="font-size:10.5px;color:var(--text-3);letter-spacing:.08em;text-transform:uppercase">Jam
                            Masuk</th>
                        <th class="text-left px-4 py-3 font-semibold whitespace-nowrap"
                            style="font-size:10.5px;color:var(--text-3);letter-spacing:.08em;text-transform:uppercase">Jam
                            Pulang</th>
                        <th class="text-center px-4 py-3 font-semibold whitespace-nowrap"
                            style="font-size:10.5px;color:var(--text-3);letter-spacing:.08em;text-transform:uppercase">Hari
                        </th>
                        <th class="text-center px-5 py-3 font-semibold whitespace-nowrap"
                            style="font-size:10.5px;color:var(--text-3);letter-spacing:.08em;text-transform:uppercase">
                            Aksi</th>
                    </tr>
                </thead>
                <tbody>
                    @forelse($assignments as $group)
                        @php
                            ...
                            $onTimes = $group->on_times
                                ->filter()
                                ->map(fn($t) => \Carbon\Carbon::parse($t)->format('H:i'))
                                ->unique()
                                ->sort()
                                ->values();
                            $offTimes = $group->off_times
                                ->filter()
                                ->map(fn($t) => \Carbon\Carbon::parse($t)->format('H:i'))
                                ->unique()
                                ->sort()
                        ...
                        <tr class="assign-row" style="border-bottom:1px solid var(--border)">
                            ...
                            <td class="px-5 py-3.5 whitespace-nowrap">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        <p style="font-size:13.5px;font-weight:700;color:var(--text-1)">{{ $emp->name }}
                                        </p>
                     ...
                     color:{{ $isOff ? '#94a3b8' : '#a78bfa' }}">
                                ...
                                @if ($group->new_shifts->isNotEmpty())
                                    <div class="flex flex-nowrap gap-1">
                                        @foreach ($group->new_shifts as $ns)
                                            ...
                                            <span class="px-2 py-0.5 rounded-md whitespace-nowrap"
                         ...
                         color:{{ $nsOff ? '#94a3b8' : '#fb923c' }}">
                            ...
                            <td class="px-4 py-3.5 whitespace-nowrap">
                                ...
                                <p style="font-size:11px;color:var(--text-3)">{{ $minDate->translatedFormat('D') }} –
                                    {{ $maxDate->translatedFormat('D') }}</p>
                            </td>
                                ...
                                @if ($onTimes->isNotEmpty())
                                    <div class="flex flex-col gap-0.5">
                                        @foreach ($onTimes as $t)
                                            <span
                                                style="font-size:13px;font-weight:700;color:#22c55e;font-family:monospace">↓
                                                {{ $t }}</span>
                                        @endforeach
                                    </div>
                                @else
                            ...
                            <td class="px-5 py-3.5 text-center">
                                <button
                                    class="expand-trigger ib-bg w-8 h-8 rounded-lg flex items-center justify-center mx-auto"
                        ...
                        <tr id="detail-{{ $loop->index }}" style="display:none">
                            <td colspan="8" style="padding:0;background:var(--bg-ghost)">
                                <table class="w-full" style="border-collapse:collapse">
                                    <thead>
                                        <tr style="border-bottom:1px solid var(--border)">
                                            <th class="text-left px-8 py-2 font-semibold"
                                                style="font-size:10px;color:var(--text-3);letter-spacing:.07em;text-transform:uppercase;width:180px">
                                                Tanggal</th>
                                            <th class="text-left px-4 py-2 font-semibold"
                                                style="font-size:10px;color:var(--text-3);letter-spacing:.07em;text-transform:uppercase">
                                                Shift Code</th>
                                            <th class="text-left px-4 py-2 font-semibold"
                                                style="font-size:10px;color:var(--text-3);letter-spacing:.07em;text-transform:uppercase">
                                                New Shift</th>
                                            ...
                                            <th class="text-left px-4 py-2 font-semibold"
                                                style="font-size:10px;color:var(--text-3);letter-spacing:.07em;text-transform:uppercase">
                                                Jam Pulang</th>
                                            <th class="text-center px-4 py-2 font-semibold"
                                                style="font-size:10px;color:var(--text-3);letter-spacing:.07em;text-transform:uppercase">
                                                Edit</th>
                                        </tr>
                                    </thead>
                                        ...
                                        @foreach ($group->rows->sortBy('date') as $row)
                                            ...
                                            <tr style="border-bottom:1px solid var(--border)">
                                                <td class="px-8 py-2.5">
                                                    <p style="font-size:12.5px;font-weight:600;color:var(--text-2)">
                                                        {{ \Carbon\Carbon::parse($row->date)->format('d M Y') }}</p>
                                                    <p style="font-size:11px;color:var(--text-3)">
                                                        {{ \Carbon\Carbon::parse($row->date)->translatedFormat('l') }}</p>
                                                </td>
                                                ...
                                                <td class="px-4 py-2.5">
                                                    @if (!$isOff && $activeShift?->on_time)
                                                        <span
                                                            style="font-size:12px;font-weight:700;color:#22c55e;font-family:monospace">↓
                                                            {{ \Carbon\Carbon::parse($activeShift->on_time)->format('H:i') }}</span>
                                                    @else
                                                        <span style="color:var(--text-3);font-size:12px">—</span>
                                                    @endif
                                                </td>
                                                <td class="px-4 py-2.5">
                                                    @if (!$isOff && $activeShift?->off_time)
                                                        <span
                                                            style="font-size:12px;font-weight:700;color:#60a5fa;font-family:monospace">↑
                                                            {{ \Carbon\Carbon::parse($activeShift->off_time)->format('H:i') }}</span>
                                                    @else
                                                        <span style="color:var(--text-3);font-size:12px">—</span>
                                                    @endif
                                                </td>
                                                <td class="px-4 py-2.5 text-center">
                                                    <button
                                                        class="edit-row-btn ib-bg w-7 h-7 rounded-lg flex items-center justify-center mx-auto"
                                                        data-id="{{ $row->id }}"
                    ...
                    @empty
                        <tr>
                            <td colspan="8" class="px-5 py-16 text-center">
                                ...
                                <p style="font-size:12px;color:var(--text-3);margin-top:4px;opacity:.6">Gunakan tombol
                                    Assign atau Import untuk menambahkan jadwal</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
        ...
        @if ($assignments->hasPages())
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 px-5 py-4"
                ...
                <p style="font-size:13px;color:var(--text-3)">
                    Menampilkan <strong
                        style="color:var(--text-2)">{{ $assignments->firstItem() }}–{{ $assignments->lastItem() }}</strong>
                    dari <strong style="color:var(--text-2)">{{ number_format($assignments->total()) }}</strong> karyawan
                </p>
