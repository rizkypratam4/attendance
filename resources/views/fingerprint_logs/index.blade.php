...
<div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-6">
    <div>
        ...
        <p style="font-size:13px;color:var(--text-3);margin-top:5px">Raw biometric data from <span style="font-family:monospace;color:var(--text-2)">dbo.AttendanceMachinePolling</span></p>
    </div>
    <div class="flex items-center gap-2 flex-shrink-0">
        ...
        <form method="POST" action="{{ route('fingerprint.sync') }}" id="syncForm">
            ...
            <button type="submit" id="syncBtn"
                ...
                <svg id="syncIcon" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5">
                    <path d="M23 4v6h-6"/><path d="M1 20v-6h6"/>
                    <path d="M3.51 9a9 9 0 0114.85-3.36L23 10M1 14l4.64 4.36A9 9 0 0020.49 15"/>
                </svg>
                <span id="syncLabel">Sync Sekarang</span>
            </button>
        </form>
        {{-- Export --}}
...
<div class="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-5">
    ...
    <div class="card rounded-2xl p-4">
        <p style="font-size:10.5px;font-weight:600;color:var(--text-3);letter-spacing:.08em;text-transform:uppercase;margin-bottom:8px">Total Log</p>
        <p style="font-size:26px;font-weight:800;color:var(--text-1);line-height:1;font-family:monospace">{{ number_format($stats['total']) }}</p>
    </div>

    <div class="card rounded-2xl p-4">
        <p style="font-size:10.5px;font-weight:600;color:var(--text-3);letter-spacing:.08em;text-transform:uppercase;margin-bottom:8px">Belum Diproses</p>
        <p style="font-size:26px;font-weight:800;color:#f59e0b;line-height:1;font-family:monospace">{{ number_format($stats['unprocessed']) }}</p>
    </div>

    <div class="card rounded-2xl p-4">
        <p style="font-size:10.5px;font-weight:600;color:var(--text-3);letter-spacing:.08em;text-transform:uppercase;margin-bottom:8px">Sudah Diproses</p>
        <p style="font-size:26px;font-weight:800;color:#22c55e;line-height:1;font-family:monospace">{{ number_format($stats['processed']) }}</p>
    </div>

    <div class="card rounded-2xl p-4">
        <p style="font-size:10.5px;font-weight:600;color:var(--text-3);letter-spacing:.08em;text-transform:uppercase;margin-bottom:8px">Log Hari Ini</p>
        <p style="font-size:26px;font-weight:800;color:var(--text-1);line-height:1;font-family:monospace">{{ number_format($stats['today']) }}</p>
    </div>
...
<div class="card rounded-2xl p-4 mb-5">
    <form method="GET" action="{{ route('fingerprint.index') }}">
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 items-end">
            ...
            {{-- Tanggal --}}
            <div>
                <label style="font-size:10.5px;font-weight:600;color:var(--text-3);letter-spacing:.08em;text-transform:uppercase;display:block;margin-bottom:7px">Tanggal</label>
            ...
            {{-- Barcode --}}
            <div>
                <label style="font-size:10.5px;font-weight:600;color:var(--text-3);letter-spacing:.08em;text-transform:uppercase;display:block;margin-bottom:7px">Barcode</label>
                <div class="flex items-center gap-2 px-3 py-2.5 rounded-xl"
                     style="background:var(--bg-input);border:1px solid var(--border-in)">
                    <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="var(--text-3)" stroke-width="2" class="flex-shrink-0">
            ...
            {{-- Tipe --}}
            <div>
                <label style="font-size:10.5px;font-weight:600;color:var(--text-3);letter-spacing:.08em;text-transform:uppercase;display:block;margin-bottom:7px">Tipe</label>
                <div class="relative">
                    ...
                    <svg class="absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none" width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="var(--text-3)" stroke-width="2.5">
                        <path d="M6 9l6 6 6-6"/>
                    </svg>
                </div>
            </div>
            ...
            <div class="flex gap-2">
                <div class="flex-1">
                    <label style="font-size:10.5px;font-weight:600;color:var(--text-3);letter-spacing:.08em;text-transform:uppercase;display:block;margin-bottom:7px">Status</label>
                    <div class="relative">
                        <select name="processed" class="w-full px-3 py-2.5 rounded-xl appearance-none"
                            ...
                            <option value="1" {{ request('processed') === '1' ? 'selected' : '' }}>Diproses</option>
                        </select>
                        <svg class="absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none" width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="var(--text-3)" stroke-width="2.5">
                            <path d="M6 9l6 6 6-6"/>
                        </svg>
...
<div class="card rounded-2xl" style="overflow:hidden">
        ...
        <table class="w-full" id="logsTable" style="border-collapse:collapse;min-width:750px">
            <thead>
                <tr style="border-bottom:1px solid var(--border)">
                    <th class="text-left px-5 py-3 font-semibold" style="font-size:10.5px;color:var(--text-3);letter-spacing:.08em;text-transform:uppercase">#</th>
                    <th class="text-left px-4 py-3 font-semibold" style="font-size:10.5px;color:var(--text-3);letter-spacing:.08em;text-transform:uppercase">Barcode</th>
                    <th class="text-left px-4 py-3 font-semibold" style="font-size:10.5px;color:var(--text-3);letter-spacing:.08em;text-transform:uppercase">Karyawan</th>
                    ...
                    <th class="text-left px-4 py-3 font-semibold" style="font-size:10.5px;color:var(--text-3);letter-spacing:.08em;text-transform:uppercase">Waktu</th>
                    <th class="text-left px-4 py-3 font-semibold" style="font-size:10.5px;color:var(--text-3);letter-spacing:.08em;text-transform:uppercase">Tipe</th>
                    <th class="text-left px-4 py-3 font-semibold" style="font-size:10.5px;color:var(--text-3);letter-spacing:.08em;text-transform:uppercase">Status</th>
            ...
            <tbody>
                ...
                <tr class="log-row" style="border-bottom:1px solid var(--border)">
                    ...
                    <td class="px-4 py-3.5">
                        @if($log->employee)
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    <p style="font-size:13.5px;font-weight:700;color:var(--text-1)">{{ $log->employee->name }}</p>
                                    <p style="font-size:11px;color:var(--text-3)">{{ $log->employee->department ?? '—' }}</p>
                                </div>
                            </div>
                        @else
                            <div class="flex items-center gap-2">
                                <div class="w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0"
                                    ...
                                    <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#f87171" stroke-width="2.5">
                                        <circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/>
                                    </svg>
                                </div>
                    ...
                    <td class="px-5 py-3.5 text-right">
                        <button class="log-trigger ib-bg w-8 h-8 rounded-lg flex items-center justify-center ml-auto"
                                data-id="{{ $log->id }}"
                                data-barcode="{{ $log->barcode }}"
                ...
                <tr>
                    <td colspan="8" class="px-5 py-16 text-center">
                        <div style="color:var(--text-3)">
                            <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" style="margin:0 auto 12px;opacity:.3">
                                <path d="M3 5h18M3 10h18M3 15h18M3 20h18"/>
                            </svg>
                            <p style="font-size:14px;font-weight:600;color:var(--text-3)">Tidak ada data</p>
                            <p style="font-size:12px;margin-top:4px;opacity:.6">Coba ubah filter atau sync data dari mesin</p>
                        </div>
                    </td>
                </tr>
                @endforelse
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 px-5 py-4"
        ...
        <p style="font-size:13px;color:var(--text-3)">
            Showing <strong style="color:var(--text-2)">{{ $logs->firstItem() }}–{{ $logs->lastItem() }}</strong> of
        ...
        <div class="flex items-center gap-1">
            ...
            @else
                <a href="{{ $logs->previousPageUrl() }}" class="w-8 h-8 rounded-lg flex items-center justify-center ib-bg">
                    <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M15 18l-6-6 6-6"/></svg>
                </a>
            @endif
            ...
            @if($logs->hasMorePages())
                <a href="{{ $logs->nextPageUrl() }}" class="w-8 h-8 rounded-lg flex items-center justify-center ib-bg">
...
.log-dd-item:hover { background: var(--bg-hover); color: var(--text-1); }
.log-dd-danger { color: #f87171 !important; }
.log-dd-danger:hover { background: rgba(239,68,68,.12) !important; }
