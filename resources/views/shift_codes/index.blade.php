    ...
    <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-6">
        <div>
            ...
            <p style="font-size:13px;color:var(--text-3);margin-top:5px">
                Konfigurasi dan kelola jam kerja standar untuk tim Anda.
            </p>
        </div>
        ...
        <button onclick="openM('mCreateShiftCode')"
            ...
            Tambah Kode Shift Baru
        </button>
    </div>

    <div class="card rounded-2xl px-4 py-3 mb-5 flex items-center gap-3">
        <div class="flex items-center gap-2 flex-1 px-3 py-2 rounded-xl"
            ...
            <input type="text" id="shiftSearch" placeholder="Cari berdasarkan kode..."
        ...
        <div class="relative flex-shrink-0">
            ...
            <svg class="absolute right-2.5 top-1/2 -translate-y-1/2 pointer-events-none" width="12" height="12"
                viewBox="0 0 24 24" fill="none" stroke="var(--text-3)" stroke-width="2.5">
                <path d="M6 9l6 6 6-6" />
            </svg>
        </div>
                ...
                <tbody>
                    @forelse($shiftCodes as $shiftCode)
                        <tr class="shift-row" style="border-bottom:1px solid var(--border)"
                            data-code="{{ $shiftCode->code }}" data-name="{{ $shiftCode->shift->name ?? '' }}"
                            ...
                            {{ $shiftCode->is_day_off ? 'background:rgba(251,146,60,.15);color:#fb923c' : 'background:rgba(34,197,94,.15);color:#22c55e' }}">
                                    {{ $shiftCode->is_day_off ? 'Hari Libur' : 'Hari Kerja' }}
                                </span>
                            </td>
                            <td class="px-4 py-4">
                                <span
                                    class="px-2 py-1 rounded-full text-xs font-medium {{ $shiftCode->has_idt ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                            ...
                            <td class="px-5 py-4 text-right whitespace-nowrap">
                                <div class="flex items-center justify-center gap-2">
                                    <button
                                        onclick="openEditShiftCode({{ $shiftCode->id }}, '{{ $shiftCode->code }}', '{{ $shiftCode->shift_id }}', {{ $shiftCode->has_idt ? 1 : 0 }})"
                                    ...
                                    <button
                                        onclick="openDeleteShiftCode('{{ $shiftCode->code }}', '{{ $shiftCode->id }}')"
                                    ...
                                    <form id="delete-form-shift-code-{{ $shiftCode->id }}"
                                        action="{{ route('shift_codes.destroy', $shiftCode->id) }}" method="POST"
                                        class="hidden">
                                        @csrf
                                        @method('DELETE')
                                    </form>
                    ...
                    @empty
                        <tr>
                            <td colspan="8" class="py-8 px-6 text-center text-gray-500"
                                ...
                                Tidak ada kode shift ditemukan.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
