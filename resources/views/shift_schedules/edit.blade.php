<x-ui.modal id="mEditShiftSchedule" title="Edit Shift Schedule" maxWidth="540px">
    <form id="editShiftScheduleForm" method="POST">
        @csrf
        @method('PUT')
        <div class="space-y-4">
            <div>
                ...
                <select name="shift_code_id" class="minput" style="cursor:pointer">
                    ...
                    @foreach(\App\Models\ShiftCode::with('shift')->get() as $code)
            ...
            <div>
                ...
                <input name="schedule_code" type="text" class="minput">
                @error('schedule_code')
                    <p class="text-red-400 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
        ...
        <div class="flex gap-3 mt-6">
            <button type="button" onclick="closeM('mEditShiftSchedule')" class="flex-1 py-2.5 rounded-xl font-medium"
                style="font-size:14px;border:1px solid var(--border);background:var(--bg-ghost);color:var(--text-2);cursor:pointer">Cancel</button>
            <button type="submit" class="flex-1 purbtn py-2.5 rounded-xl font-semibold" style="font-size:14px">Update Schedule</button>
        </div>
    </form>
</x-ui.modal>